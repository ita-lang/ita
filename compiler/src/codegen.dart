/// Code Generator: Glu AST → Dart Kernel AST → .dill
///
/// Transforma a AST da linguagem Glu em nós do Dart Kernel,
/// prontos para serialização em formato binário .dill.
///
/// Mapeamento de tipos:
///   struct   → Class (fields final, constructor com named params)
///   class    → Class (fields mutáveis, constructor, herança)
///   enum     → Classe abstrata + subclasses (uma por variant)
///   trait    → Classe abstrata
///   impl     → Métodos adicionados à classe target

import 'dart:io';
import 'package:kernel/kernel.dart' as k;
import 'package:kernel/core_types.dart';
import 'ast.dart' as glu;
import 'token.dart';
import 'lexer.dart' as lex show Lexer;
import 'parser.dart' as parse show Parser;

class CompileError {
  final String message;
  final int line;
  final int column;
  const CompileError(this.message, this.line, this.column);
...
class CodeGenerator {
  ...
  // URI do módulo
  final _fileUri = Uri.parse('file:///glutter/main.glu');
  final _libUri = Uri.parse('app:///main.glu');
  ...
  String? _resolveModulePath(String module) {
    ...
    // Tentar: module.glu, module/mod.glu
    final candidates = [
      '$sourceDir/$module.glu',
      '$sourceDir/$module/mod.glu',
    ];
  ...
  bool _shouldImport(String? name, bool isPublic, List<glu.ImportMember>? filter) {
    ...
    if (filter == null) return true; // importar tudo
  ...
  /// glob("*.glu") → Directory(".").listSync + filter
  ...
  k.Expression _compileClosure(glu.ClosureExpr expr) {
    ...
    if (expr.params.isEmpty) {
      // Trailing closure sem params explícitos — detectar $0, $1 usage
      // Pré-declarar $0 e $1 como parâmetros implícitos
      for (var i = 0; i < 3; i++) {
  ...
  void _error(String msg, int line, int col) {
    errors.add(CompileError(msg, line, col));
  }
}
